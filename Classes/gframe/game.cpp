...
*	}//start ygocore when mobile is in landscape mode, or using Android tablets or TV.*/
	...
	//main menu
	wchar_t strbuf[256];
	myswprintf(strbuf, L"NanahiraMobile Version:%X.0%X.%X", PRO_VERSION >> 12, (PRO_VERSION >> 4) & 0xff, PRO_VERSION & 0xf);
#ifdef _IRR_ANDROID_PLATFORM_
	...
	lstHostList->setItemHeight(40 * yScale);
...
//dont merge
	...
	//tip for cards in select / display list
 	stCardListTip = env->addStaticText(L"", rect<s32>(0, 0, 150 * xScale, 150 * yScale), false, true, wCardSelect, -1, true);
